...
namespace Algorithm_ID3
{
    ...
    */
    ...
    public class Data
    {
        ...
        {
            AlgorythmID3(Attr, Rows);
        }

        private void AlgorythmID3(List<string> attr, List<Dictionary<string, string>> rows, 
            string branch = null, int index = 1)
        {
            //Якщо кількість атрибутів = 1, то значить що залишилася лише децизія і пора переривати рекуренцію
            if (attr.Count == 1)
            {
                //Перевірка чи залишилися необробленні рядки
                if(rows.Count == 0)
                    return;
                else
            ...
            double infoAll = Info(attr, rows);
            Info2(attr, rows);
            int maxIndex = infos.IndexOf(infos.Min());
            ...
            foreach (var group in groups)
            {
                ...
                else
                {
                    ...
                    AlgorythmID3(newAttr, selectedRows, newBranch, index+1);
                }
            }
        }

        private void Info2(List<string> attr2, List<Dictionary<string, string>> rows)
        {
            infos = new List<double>();
            string attr;
            for (int i = 0; i < attr2.Count-1; i++)
            {
                ...
                if(attr != null)
                {
                    IEnumerable<IGrouping<string, Dictionary<string, string>>> groups = rows.GroupBy(x => x[attr]);
                    foreach (var group in groups)
                    {
                        List<Dictionary<string, string>> selectedRows = group.Select(x => x).ToList();
                        var groupedItem = selectedRows.GroupBy(x => x[attr2.LastOrDefault()]);
        ...
        {
            double rez = 0;
            var decyzja = attr.LastOrDefault();
            if (decyzja != null)
            {
                ...
                foreach (var item in list_dec)
                {
                    ...
                    {
                        firstTr = true;
                        rez = -tmp * Math.Log(tmp, 2);
                    }
                    else rez -= tmp * Math.Log(tmp, 2);
                }
                return rez;
            }
            return 0;
